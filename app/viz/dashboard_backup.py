...
class DenseLayer {
  ...
  forward(x, activate) {
    ...
    for (var r = 0; r < this.outDim; r++) {
      ...
      this.lastOut = this.lastOut || new Float32Array(this.outDim);
    }
    this.lastOut = out.slice();
    if (activate) for (var i = 0; i < out.length; i++) this.lastOut[i] = out[i];
    return out;
  }
...
class QNetwork {
  constructor() {
    this.l1 = new DenseLayer(32, 128);   // 32-dim state input
  ...
  backward(target_q, action) {
    var q = this.forward(this._lastState);
  ...
  train(state, action, target_q, lr) {
    ...
    var loss = this.backward(target_q, action);
    this.adamT++;
...
var DQN = {
  ...
  /* ── CHANGED #1: saveModel — POST to /save_weights on server ── */
  ...
  /* ── CHANGED #2: loadModel — GET from /load_weights on server ── */
  ...
  /* save every 50 train steps to avoid excessive writes */
  maybeSave: function() {
...
extractState: function(obs) {
    ...
    // [0–11] Queue lengths — already normalised [0,1] by the env. DO NOT divide again.
    var ql = obs.queue_lengths || [];
    for (var i = 0; i < 12; i++) state[i] = Math.min(Math.max(ql[i] || 0, 0), 1);

    // [12–15] Phase one-hot — [NS_GREEN, EW_GREEN, ALL_RED, NS_MINOR]
    var ph = obs.phase_onehot || [1, 0, 0, 0];
    for (var i = 0; i < 4; i++) state[12 + i] = ph[i] || 0;

    // [16] Phase elapsed (already normalised [0,1])
    state[16] = Math.min(Math.max(obs.phase_elapsed_norm || 0, 0), 1);

    // [17] Pressure differential: env returns [-1,1], map to [0,1] for stable training
    state[17] = ((obs.pressure_differential || 0) + 1) / 2;

    // [18] Average delay (already normalised [0,1] by env: delay/120s)
    state[18] = Math.min(Math.max(obs.avg_delay_norm || 0, 0), 1);

    // [19] Fairness score [0,1] — 0=perfect fairness, 1=maximally unfair
    state[19] = Math.min(Math.max(obs.fairness_score || 0, 0), 1);

    // [20–31] Spillback flags — binary 0/1 per lane
  ...
  computeReward: function(info) {
    if (!info) return 0;
    var r = 0;
    r += (info.step_cleared || 0) * 1.0;
    r -= ((info.ns_queue || 0) + (info.ew_queue || 0)) * 0.05;
    r -= (info.step_crashes || 0) * 5.0;
    r -= (info.avg_delay || 0) * 0.02;
...
// Maps DQN's 3 internal actions to the 5-action API action space.
// DQN action 0 = HOLD    → API 0 (MAINTAIN)
// DQN action 1 = SWITCH  → API 1 (SWITCH_PHASE)
// DQN action 2 = EXTEND  → API 2 (EXTEND_GREEN)   ← was broken (mapped to 0)
function dqnActionToApiAction(dqnAction, obs) {
  var map = [0, 1, 2];   // direct 1-to-1 for the three controllable actions
...
function updateVehicles(dt) {
  LANE_NAMES.forEach(function(ln) {
    var def=LDEFS[ln]; if(!def) return; var q=LQUEUES[ln]; var green=carIsGreen(ln),stop=def.stopPos; var fwd=(def.dir==='S'||def.dir==='E'); var yieldBlock=crossingDirInBox(ln[0]);
    for(var i=0;i<q.length;i++){
      var car=q[i]; if(car.crashed||car.exited) continue; if(car.alpha<1) car.alpha=Math.min(1,car.alpha+0.07);
      var ahead=i>0&&!q[i-1].exited&&!q[i-1].crashed?q[i-1]:null; var blockPos;
      if(fwd){blockPos=ahead?(ahead.pos-SAFE):Infinity;var pastStop=car.pos>=stop;if(!pastStop&&(!green||yieldBlock))blockPos=Math.min(stop,blockPos);if(car.moving){var dist=blockPos-car.pos;if(dist>0.5)car.pos+=Math.min(CAR_SPEED*dt,dist-0.5);else car.moving=false;}else if(blockPos-car.pos>SAFE*0.4)car.moving=true;if(green&&!car.counted&&car.pos>stop&&!yieldBlock){car.counted=true;addVizPass(1);}if(car.pos>def.exitPos)car.exited=true;}
      else{blockPos=ahead?(ahead.pos+SAFE):-Infinity;var pastStop=car.pos<=stop;if(!pastStop&&(!green||yieldBlock))blockPos=Math.max(stop,blockPos);if(car.moving){var dist=car.pos-blockPos;if(dist>0.5)car.pos-=Math.min(CAR_SPEED*dt,dist-0.5);else car.moving=false;}else if(car.pos-blockPos>SAFE*0.4)car.moving=true;if(green&&!car.counted&&car.pos<stop&&!yieldBlock){car.counted=true;addVizPass(1);}if(car.pos<def.exitPos)car.exited=true;}
    }
    for(var i=q.length-1;i>=0;i--){if(q[i].crashed||q[i].exited)q.splice(i,1);}
  });
  checkCrashes(); LANE_NAMES.forEach(function(ln){for(var i=LQUEUES[ln].length-1;i>=0;i--){if(LQUEUES[ln][i].crashed)LQUEUES[ln].splice(i,1);}});
  for(var i=crashEvents.length-1;i>=0;i--){crashEvents[i].t+=dt/30;if(crashEvents[i].t>1)crashEvents.splice(i,1);}
...
var _carColorMap={};
var _CAR_PALETTE=[
  new BABYLON.Color3(0.85,0.15,0.15), // red
  new BABYLON.Color3(0.12,0.38,0.72), // deep blue
  new BABYLON.Color3(0.88,0.88,0.88), // silver
  new BABYLON.Color3(0.06,0.06,0.07), // black
  new BABYLON.Color3(0.92,0.72,0.12), // yellow taxi
  new BABYLON.Color3(0.18,0.52,0.24), // green
  new BABYLON.Color3(0.55,0.25,0.62), // purple
  new BABYLON.Color3(0.92,0.42,0.10), // orange
  new BABYLON.Color3(0.28,0.56,0.78), // sky blue
  new BABYLON.Color3(0.70,0.70,0.68), // light grey
  new BABYLON.Color3(0.48,0.18,0.10), // maroon
  new BABYLON.Color3(0.82,0.82,0.20), // lime
];
...
function buildBabylonScene() {
  ...
  // Much farther fog — no more vanishing surroundings
  ...
  // Sky gradient via skydome
  var skyMat = new BABYLON.StandardMaterial('skyMat', _babScene);
  skyMat.backFaceCulling = false;
  skyMat.disableLighting = true;
  // Use a large sphere for sky
  ...
  // Hemisphere ambient — warm ground, cool sky
...
function buildRoadGeometry(scene, shadowGen) {
  ...
  // Grass zones in corner quadrants
  ...
  // Intersection box — slightly lighter worn tarmac
...
function buildBuildings(scene, shadowGen) {
  var configs = [
    // Corner shops/low-rise near intersection
  ...
  configs.forEach(function(bc, idx) {
    ...
    // Rooftop HVAC + water tower for tall buildings
    if (bc.h > 20) {
      ...
      if (bc.h > 50) {
        ...
        var blinkT = Math.random() * Math.PI * 2;
        scene.registerBeforeRender(function() {
          blinkT += 0.04;
          var on = Math.sin(blinkT) > 0.6;
          blinkLight.intensity = on ? 1.5 : 0;
          blinkMat.emissiveColor = on ? new BABYLON.Color3(1, 0, 0) : new BABYLON.Color3(0.2, 0, 0);
        });
      }
    }
    ...
    // Windows
    if (bc.style === 'glass') {
      var wCols = Math.max(3, Math.floor(bc.w / 1.6));
      var wRows = Math.max(4, Math.floor(bc.h / 2.0));
      for (var wr = 0; wr < wRows; wr++) {
        for (var wc = 0; wc < wCols; wc++) {
          var lit = Math.random() > 0.22;
          var gw = BABYLON.MeshBuilder.CreateBox('gw' + idx + wr + '_' + wc,
            { width: (bc.w / wCols) * 0.82, height: 1.55, depth: 0.04 }, scene);
          gw.position = new BABYLON.Vector3(
            bc.x - bc.w / 2 + (wc + 0.5) * (bc.w / wCols),
            1.8 + wr * (bc.h / wRows),
            bc.z + bc.d / 2 + 0.05);
          var gwMat = lit ? winGlassDayMat : winOffMat;
          gw.material = gwMat;
        }
      }
    } else {
      var wCols = Math.max(2, Math.floor(bc.w / 2.2));
      var wRows = Math.max(2, Math.floor(bc.h / 3.0));
      for (var wr = 0; wr < wRows; wr++) {
        for (var wc = 0; wc < wCols; wc++) {
...
function buildTrees(scene, shadowGen) {
  ...
  treePosns.forEach(function(tp, ti) {
    ...
    // 4-sphere foliage cluster per tree for more volume
...
function buildTrafficLights(scene,shadowGen) {
  ...
  /* 4 poles, one per direction, positioned at kerb corners */
  var tl_positions=[
...
function buildStreetFurniture(scene,shadowGen) {
  /* Street lamp poles — steel grey with warm glow heads */
  ...
  /* Pedestrian bollards — black/yellow at corners */
  ...
  /* Pedestrian crossing signal boxes */
...
function buildClouds(scene) {
  ...
  cloudPositions.forEach(function(cp, ci) {
    ...
    for (var ci2 = 0; ci2 < 4; ci2++) {
      ...
      // Slowly drift clouds
      var driftSpeed = 0.0008 + Math.random() * 0.0006;
      var origX = cloud.position.x;
      scene.registerBeforeRender(function() {
        cloud.position.x += driftSpeed;
        if (cloud.position.x > origX + 40) cloud.position.x = origX - 40;
      });
    }
  });
}
...
function updateBabylonCars() {
  ...
  // Only show cars within ~200px of centre on the canvas
  const VISIBLE_RANGE = 200;
  const posScale = 28 / VISIBLE_RANGE;

  // Reuse a single Vector3 per mesh to avoid GC pressure
  var laneCarList = {};
  LANE_NAMES.forEach(function(ln) {
  ...
  _babCarMeshes.forEach(function(entry) {
    ...
    if (def.dir === 'S' || def.dir === 'N') {
      ...
      // Hide car if it's far outside the visible road segment
      if (Math.abs(babZ) > 32) {
        mesh.root.setEnabled(false);
        return;
      }
    ...
    // Update position in-place instead of allocating new Vector3 every frame
...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   Runs two independent sessions simultaneously on the same task+seed.
   Fixed uses a 30s cycle timer. AI uses the loaded DQN with ε=0 (pure exploit).
═══════════════════════════════════════════════════════════════════════ */
var BATTLE = {
  running: false,
  fixedSessionId: null,
  ...
  async start() {
    ...
    this.fixedTimer = 0;
    this.fixed = { cleared: 0, arrived: 0, totalWait: 0, waitCount: 0, los: '—', step: 0 };
    this.ai    = { cleared: 0, arrived: 0, totalWait: 0, waitCount: 0, los: '—', step: 0 };
    this.resetUI();

    /* create two sessions — same task, same seed */
    try {
  ...
  async tick() {
    ...
    if (!done1) {
      ...
      promises.push(
        fetch(BASE+'/step',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({session_id:this.fixedSessionId,action:fixedAction})}).then(r=>r.json())
      );
    } else { promises.push(Promise.resolve(null)); }

    if (!done2) {
      /* DQN AI action — pure exploit, ε=0 */
      var aiAction = 0;
      if (DQN.online) {
        var tmpState = DQN.extractState(this._aiLastObs || {});
        var qv = DQN.online.forward(tmpState);
        aiAction = qv[0] > qv[1] ? (qv[0] > qv[2] ? 0 : 2) : (qv[1] > qv[2] ? 1 : 2);
        aiAction = dqnActionToApiAction(aiAction, {});
      }
      promises.push(
        fetch(BASE+'/step',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({session_id:this.aiSessionId,action:aiAction})}).then(r=>r.json())
      );
    } else { promises.push(Promise.resolve(null)); }
    ...
    if (fixedSd) {
      ...
      this.fixed.arrived = fi.total_arrived || 0;
    ...
    if (aiSd) {
      ...
      this.ai.arrived = ai.total_arrived || 0; 
    ...
    this.updateUI();
    if ((fixedSd && fixedSd.done) && (aiSd && aiSd.done)) { this.finish(); }
  },
  ...
  finish() {
    ...
    var aiEff   = this.ai.cleared    > 0 ? (this.ai.cleared    / Math.max(this.ai.cleared    + 1, 1)) * 100 : 0;
    var fixEff  = this.fixed.cleared > 0 ? (this.fixed.cleared / Math.max(this.fixed.cleared + 1, 1)) * 100 : 0;
    ...
    var aiScore  = (this.ai.cleared    / Math.max(this.horizon, 1)) - (aiWait  / 80);
    var fixScore = (this.fixed.cleared / Math.max(this.horizon, 1)) - (fixWait / 80);
  ...
  updateUI() {
    ...
    /* Fixed side */
    var fixEff = this.fixed.arrived > 0 ? Math.min(100, (this.fixed.cleared / this.fixed.arrived * 100)).toFixed(0) + '%' : '—';
    st('fixed-eff',     fixEff);
    ...
    var fcb = document.getElementById('fixed-cleared-bar'); if(fcb) fcb.style.width = Math.min(100, this.fixed.cleared / Math.max(this.horizon * 0.05, 1) * 100) + '%';

    /* AI side */
    var aiEff = this.ai.arrived > 0 ? Math.min(100, (this.ai.cleared / this.ai.arrived * 100)).toFixed(0) + '%' : '—';
    st('ai-eff',     aiEff);
    st('ai-wait',    aiWait);
    ...
    var acb = document.getElementById('ai-cleared-bar'); if(acb) acb.style.width = Math.min(100, this.ai.cleared / Math.max(this.horizon * 0.05, 1) * 100) + '%';
...
/* ═══════════════════════════════════════════════════════════════════════
   STARTUP — CHANGED #3: load from server, no window.storage
═══════════════════════════════════════════════════════════════════════ */
(async function startup() {
  ...
  log('Dashboard ready — DQN agent ' + (restored ? 'restored from server checkpoint' : 'initialised fresh') + ' (32→128→128→64→3).', 'rl');
  log('Architecture: Double DQN · Input(32) · Adam lr=0.0005 · γ=0.95 · Replay 2000 · Batch 32 · ε-greedy', 'rl');
